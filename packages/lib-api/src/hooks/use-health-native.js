...
export function useHealth() {
    ...
    useEffect(() => {
        if (summaryData) {
            store.setHealthSummary(summaryData);
        }
        else {
    ...
    const checkPermissions = async () => {
        try {
            ...
            if (Platform.OS === 'ios') {
                ...
                const granted = await requestPermission(ANDROID_HEALTH_PERMISSIONS);
    ...
    const getHealthData = async (_dataType, _startDate, _endDate) => {
        // This would need to be implemented with native Health Kit/Connect APIs
        // For now, return empty array
        return [];
    };
    const syncHealthData = async (dataTypes = HEALTH_DATA_TYPES) => {
        ...
        try {
            ...
            const dataRequests = await Promise.all(dataTypes.map(async (dataType) => {
