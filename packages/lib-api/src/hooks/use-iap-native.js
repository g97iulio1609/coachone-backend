...
export function useIAP() {
    ...
    useEffect(() => {
        const purchaseUpdateSubscription = purchaseUpdatedListener(async (purchase) => {
            const receipt = 'transactionReceipt' in purchase ? purchase.transactionReceipt : purchase.transactionId;
            if (receipt) {
        ...
        try {
            ...
            const mappedProducts = fetchedProducts.map((product) => {
                ...
                return {
                    ...
                    localizedPrice: product.localizedPrice ?? price,
                    title: product.title ?? productId,
                    description: product.description ?? '',
                    subscriptionPeriod: getSubscriptionPeriod(productId),
                };
            });
    ...
    const handlePurchaseUpdate = async (purchase) => {
        try {
            ...
            const transactionReceipt = ('transactionReceipt' in purchase ? purchase.transactionReceipt : transactionId) ||
                '';
    ...
    const restorePurchases = useCallback(async () => {
        try {
            ...
            const purchases = availablePurchases.map((purchase) => {
                ...
                const purchaseToken = 'purchaseToken' in purchase ? String(purchase.purchaseToken || '') : undefined;
                return {
                    receipt,
                    productId,
                    platform: Platform.OS,
...
// Helper function
function getSubscriptionPeriod(productId) {
    const mapping = PRODUCT_TO_PLAN_MAP[productId];
    if (!mapping)
        return undefined;
    if (mapping.cycle === 'monthly')
        return 'monthly';
    if (mapping.cycle === 'yearly')
        return 'yearly';
    return undefined;
}
