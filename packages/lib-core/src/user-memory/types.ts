...
export interface MemoryHistoryItem {
  ...
  metadata?: {
    source?: string;
    importance?: number; // 0-1, for filtering/prioritization
  };
}
...
export interface MemoryInsight {
  ...
  generatedAt: string;
  expiresAt?: string; // Optional expiration for time-sensitive insights
}
...
export interface ProjectsMemory extends DomainMemory {
  ...
  patterns: Array<
    MemoryPattern & { type: 'PROJECT_PATTERN' | 'MILESTONE_PATTERN' | 'COMPLETION_PATTERN' }
  >;
  history: Array<
    MemoryHistoryItem & { type: 'PROJECT_CREATED' | 'PROJECT_COMPLETED' | 'MILESTONE_REACHED' }
  >;
}
...
export interface TasksMemory extends DomainMemory {
  ...
  patterns: Array<
    MemoryPattern & { type: 'TASK_PATTERN' | 'COMPLETION_PATTERN' | 'PRIORITY_PATTERN' }
  >;
  history: Array<MemoryHistoryItem & { type: 'TASK_CREATED' | 'TASK_COMPLETED' | 'TASK_UPDATED' }>;
}
...
export interface HabitsMemory extends DomainMemory {
  ...
  patterns: Array<
    MemoryPattern & { type: 'HABIT_PATTERN' | 'STREAK_PATTERN' | 'COMPLETION_PATTERN' }
  >;
  history: Array<
    MemoryHistoryItem & {
...
export interface GeneralMemory extends DomainMemory {
  ...
  patterns: Array<
    MemoryPattern & { type: 'COMMUNICATION_PATTERN' | 'USAGE_PATTERN' | 'PREFERENCE_PATTERN' }
  >;
  history: Array<MemoryHistoryItem & { type: 'INTERACTION' | 'FEEDBACK' | 'PREFERENCE_CHANGE' }>;
}
...
export interface ProgressEventData {
  ...
  period: string; // e.g., "1 month", "3 months"
}
