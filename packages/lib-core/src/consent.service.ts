...
export class ConsentService {
   ...
   */
  static async giveConsent(params: CreateConsentParams): Promise<void> {
   ...
   */
  static async withdrawConsent(userId: string, policyType: PolicyType): Promise<void> {
    const policy = await PolicyService.getPolicyByType(policyType);
   ...
   */
  static async hasConsent(userId: string, policyType: PolicyType): Promise<boolean> {
    const policy = await PolicyService.getPolicyByType(policyType);
   ...
   */
  static async getUserConsents(userId: string): Promise<UserConsentStatus[]> {
    ...
    type ConsentWithPolicy = Prisma.user_consentsGetPayload<{
      include: { policies: true };
    }>;
    return consents.map((consent: ConsentWithPolicy) => ({
   ...
   */
  static async areRequiredPoliciesPublished(): Promise<boolean> {
