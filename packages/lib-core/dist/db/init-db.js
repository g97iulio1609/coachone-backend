...
async function tableExists(prisma, tableName) {
    try {
        ...
        if (error && typeof error === 'object') {
            const err = error;
            if (err.code === '42P01' || // PostgreSQL: relation does not exist
                (err.message &&
...
async function checkEssentialData(prisma) {
    try {
        const [userCount, adminCount, workoutGoalsCount, nutritionGoalsCount, policiesCount, exerciseTypesCount,] = await Promise.all([
...
export async function initializeDatabase(prisma, options) {
    ...
    try {
        ...
        const isInitialized = essentialData.hasWorkoutGoals &&
