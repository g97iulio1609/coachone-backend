...
export async function seedAuth(prisma) {
    ...
    const { admin: adminSeedResult, superAdmin: superAdminSeedResult } = await seedAdminsFromEnv(prisma);
    let admin = null;
    if (superAdminSeedResult?.admin) {
        console.warn(`✅ Super Admin ${superAdminSeedResult.created ? 'created' : 'updated'}: ${superAdminSeedResult.admin.email}`);
        admin = await prisma.users.findUnique({
            where: { email: superAdminSeedResult.admin.email },
        });
    }
    if (adminSeedResult?.admin) {
        console.warn(`✅ Admin ${adminSeedResult.created ? 'created' : 'updated'}: ${adminSeedResult.admin.email}`);
