 ...
 */
export async function createWorkoutSession(userId, request) {
    ...
    try {
        ...
        let weeks = program.weeks; // JSON from DB
        console.warn('[createWorkoutSession] Weeks type:', typeof weeks, 'isArray:', Array.isArray(weeks));
        ...
        const week = Array.isArray(weeks) ? weeks.find((w) => w.weekNumber == weekNumber) : null;
        if (!week) {
            console.error('[createWorkoutSession] Week not found:', weekNumber, 'available:', Array.isArray(weeks) ? weeks.map((w) => w.weekNumber) : 'none');
            throw new Error(`Settimana ${weekNumber} non trovata nel programma`);
        }
        // Loose equality check for dayNumber
        const day = week.days?.find((d) => d.dayNumber == dayNumber);
        if (!day) {
            console.error('[createWorkoutSession] Day not found:', dayNumber, 'available:', week.days?.map((d) => d.dayNumber));
        ...
        const exercises = day.exercises
            ? JSON.parse(JSON.stringify(day.exercises)).map((ex) => {
                if (ex.setGroups && ex.setGroups.length > 0) {
                    console.warn(`[createWorkoutSession] Hydrating setGroups for exercise ${ex.name || ex.id}`);
 ...
 */
export async function getWorkoutSessions(userId, programId, limit) {
 ...
 */
export async function getProgramSessions(programId, userId) {
 ...
 */
export async function getWorkoutProgramStats(programId, userId) {
    const sessions = await prisma.workout_sessions.findMany({
        where: {
            programId,
            userId,
 ...
 */
export async function hasSessionForDay(userId, programId, weekNumber, dayNumber) {
 ...
 */
export async function getLatestProgramSession(programId, userId) {
    const session = await prisma.workout_sessions.findFirst({
        where: {
            programId,
            userId,
