...
export function groupSelectedSets(program, weekNumber, dayNumber, exerciseId, setIndices) {
  ...
  const selectedSets = setIndices.map((idx) => allSets[idx]).filter((s) => s !== undefined);
  if (selectedSets.length === 0) {
    return program;
  }
  const avgReps = selectedSets.reduce((sum, s) => sum + (s.reps ?? 0), 0) / selectedSets.length;
  const avgWeight = selectedSets.reduce((sum, s) => sum + (s.weight ?? 0), 0) / selectedSets.length;
  const avgIntensity =
    selectedSets.reduce((sum, s) => sum + (s.intensityPercent ?? 0), 0) / selectedSets.length;
  const avgRest = selectedSets.reduce((sum, s) => sum + (s.rest ?? 0), 0) / selectedSets.length;
  const baseSet = {
