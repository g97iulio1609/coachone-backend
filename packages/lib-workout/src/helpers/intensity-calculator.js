 ...
 */
export function syncSetValues(focusField, values, oneRepMax, reps) {
    const result = {};
    ...
    switch (focusField) {
        ...
        case 'intensity':
            ...
            if (values.intensityPercent && values.intensityPercent > 0) {
                result.weight = roundTo(calculateWeightFromIntensity(oneRepMax, values.intensityPercent), 1);
            ...
            if (values.intensityPercentMax && values.intensityPercentMax > 0) {
                result.weightMax = roundTo(calculateWeightFromIntensity(oneRepMax, values.intensityPercentMax), 1);
                if (reps) {
