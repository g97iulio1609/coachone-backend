...
export class ExerciseService {
    ...
    static async searchFullText(term, options) {
        ...
        if (options.filters.approvalStatus) {
            conditions.push(Prisma.sql `e."approvalStatus" = ${options.filters.approvalStatus}::"ExerciseApprovalStatus"`);
        }
        ...
        if (options.filters.muscleIds?.length) {
            const arraySql = Prisma.sql `ARRAY[${Prisma.join(options.filters.muscleIds.map((muscleId) => Prisma.sql `${muscleId}`))}]::text[]`;
        ...
        if (options.filters.bodyPartIds?.length) {
            const arraySql = Prisma.sql `ARRAY[${Prisma.join(options.filters.bodyPartIds.map((partId) => Prisma.sql `${partId}`))}]::text[]`;
        ...
        if (options.filters.equipmentIds?.length) {
            const arraySql = Prisma.sql `ARRAY[${Prisma.join(options.filters.equipmentIds.map((equipmentId) => Prisma.sql `${equipmentId}`))}]::text[]`;
