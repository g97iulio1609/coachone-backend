...
const DEFAULT_APPROVED_STATUS = ExerciseApprovalStatus?.APPROVED ?? 'APPROVED';
const DEFAULT_PENDING_STATUS = ExerciseApprovalStatus?.PENDING ?? 'PENDING';
/**
...
export class ExerciseAdminService {
  ...
  static async generateExercisesWithAgent(options) {
    ...
    const availableBodyPartIds = metadata.bodyParts.map((bp) => `${bp.id}: ${bp.name}`).join(', ');
    const availableEquipmentIds = metadata.equipment.map((e) => `${e.id}: ${e.name}`).join(', ');
...
CRITICAL: Use ONLY the IDs listed above. Do NOT invent IDs or use names.`;
  ...
  static async generateAiPlan(prompt) {
    ...
    const exerciseTypeIdMap = metadata.exerciseTypes.map((et) => `${et.id}: ${et.name}`).join(', ');
    const modelConfig = getModelByTier('balanced');
