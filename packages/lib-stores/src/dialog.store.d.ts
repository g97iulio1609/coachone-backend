 ...
 * Dialog Store
 */
export declare const useDialogStore: import('zustand').UseBoundStore<
  Omit<import('zustand').StoreApi<DialogStore>, 'setState' | 'devtools'> & {
    setState(
      partial:
        | DialogStore
        | Partial<DialogStore>
        | ((state: DialogStore) => DialogStore | Partial<DialogStore>),
    ...
    setState(
      state: DialogStore | ((state: DialogStore) => DialogStore),
      replace: true,
      action?:
        | (
            | string
