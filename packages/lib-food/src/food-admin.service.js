  ...
  static async generateFoodsWithAgent(options) {
    ...
    const allFoods = await processBatchesInParallel({
      ...
      processor: async (batch, _batchIndex) => {
        ...
        generatedCount += (result.output?.foods || []).length;
        onProgress?.(
          generationStartProgress +
    ...
    const importRecords = allFoods
      .map((food) => {
        ...
        if (!food.description || food.description.trim().length < 10) {
          return null;
        }
        if (!food.macrosPer100g) {
          return null;
        }
        if (!food.servingSize || food.servingSize < 1 || food.servingSize > 10000) {
          return null;
        }
        // Sanitize barcode
        ...
        if (
          ...
          food.barcode.trim().length > 3 &&
          !['000000', '123456', '123456789', '111111', '999999', '0000000000000'].includes(
            food.barcode.trim()
          )
        ) {
          const trimmedBarcode = food.barcode.trim();
        ...
        const record = {
          ...
          imageUrl: normalizeUrl(food.imageUrl),
          barcode,
        };
        validatedCount++;
